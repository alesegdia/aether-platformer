...
##PROJNAME##World::##PROJNAME##World()
{
    ...
    // creation
    m_ecsWorld = std::make_shared<##PROJNAME##ECS>();
    m_ecsWorld->setCollisionTilemap(collisionTilemap);
    m_factory = std::make_shared<##PROJNAME##Factory>(m_ecsWorld->engine(), playerIndex);
...
void ##PROJNAME##World::render()
{
    ...
    al_build_transform(&tr,
                       (jojo::##PROJNAME##Config::instance().windowWidth / 2) + scroll_x,
                       140 + -3 * (jojo::##PROJNAME##Config::instance().windowHeight / 2),
                       8.0f, 8.0f, 0);
    al_use_transform(&tr);
...
void ##PROJNAME##World::update(double delta)
{
